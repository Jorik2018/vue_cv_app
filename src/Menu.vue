	...
	<div>
		<ion-menu side="start" menu-id="menuprincipal" content-id="main"  ref="menu" swipe-gesture="false" mode="md" style="z-index: 9999999;">
				...
				<ion-toolbar mode="md" v-bind:key="'menu-'+app.k">
									...
									<td style="width:50%">
										<div style="border:1px solid gray;overflow-y: hidden; width: 140px;height: 140px; border-radius: 50%;">
											<img v-bind:key="'men-'+app.ki" style="width: 100%;object-fit: cover;height: 100%;" v-bind:src="baseURL+'/fs/shami/profile/'+app.profileImg" 
												@error="$event.target.src=imgError">
										</div>
									</td>
									<td style="text-align: left;padding-left: 10px;"><div style="color:blue">Hola,<br/>{{o.display_name}}</div></td>
								</tr>
							</table>
						</div>
						<span style="right: 0px;position: absolute;top: 10px;" @click="$ionic.menuController.close('menuprincipal')"><i class="fa fa-times"></i></span>
					</ion-title>
					<ion-title  v-if="false">
						<div  class="center" v-for="(o,i) in [session.people]" v-bind:key="i"
							...
							<img style="width:100%;"  v-bind:src="baseURL+'/cdn/shami/profile/'+app.profileImg" >
							</div>
			...
			<ion-content>
				...
				<ion-item href="/logout" class="_" @click.prevent="app.logout">
					<i class="fa fa-sign-out-alt"></i>Cerrar Sessión
				</ion-item>	
				<div class="center" style="margin-top: 50px;font-size: 12px;"><b>BUILT ON: {{ app.BUILT_ON }}</b></div>
