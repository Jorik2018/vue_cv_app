<template>
	<v-map ref="map" style="height:100%" @build="mapBuild" @addlayer="addlayer">
		...
		<v-map-control class="ol-horizontal"
			...
			<v-button icon="fa-draw-polygon" @click="fit(routeLayer)" />
			<v-button icon="fa-pen" @click="drawing=!!!drawing" />
			<v-button icon="fa-save" @click="save" />
		</v-map-control>
	</v-map>
</template>
<script>
...
export default window.ui({
	...
	methods: {
		...
		mapBuild(e){
			...
			//https://gis.stackexchange.com/questions/61031/how-to-draw-a-line-between-markers-in-openlayers
			...
			addInteractions();
			console.log(me.o.plate=JSON.parse(localStorage.getItem("setting")).vehicle);
		},
		layerOnChange(evt) {
			var me = this;
			var f = evt.feature;
