        ...
        <v-form header="Estudios" action="/admin/hr/study">
            <v-table :selectable="true" row-style-class="row.synchronized?'green':(row.tmpId>0?'yellow':'')"
                ...
                <template v-slot:header>
                    ...
                    <v-button value="Crear" v-if="1 || can(perms.HR_REGISTER_STUDY)" icon="fa-plus"
                        class="on" @click.prevent="create"></v-button>
                    <v-button value="Editar" v-if="1 || can(perms.HR_REGISTER_STUDY)" icon="fa-pen"
                    ...
                    <v-button value="Eliminar" v-if="1 || can(perms.HR_REGISTER_STUDY)" icon="fa-trash"
...
export default _.ui({
    ...
    methods: {
        ...
        view(o) {
            this.open('/admin/desarrollo-social/study/' + (o.tmpId ? (-o.tmpId) : o.id));
        },
        toInt(o) {
